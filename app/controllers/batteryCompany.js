...
const { batteryBrand, validate } = require('../models/batteryCompany');
const express = require('express');
const router = express.Router();

// route to add new battery brand
router.post("/addnew", auth, async (req, res, next) => {
    const brand = new batteryBrand(req.body);
    const { error } = validate(req.body);
    if (error) return res.json({ success: false, message: 'Validation error', error: (error) });
    let find = await batteryBrand.findOne({ title: req.body.title, isDeleted: false });
...
//route get all battery brands
router.get('/all', async (req, res) => {
    batteryBrand.find({ isDeleted: false }).sort({ _id: -1 }).then(result => {
        if (result) {
...
router.get('/:id', auth, async (req, res) => {
    batteryBrand.findById(req.params.id).then(result => {
        if (result) {
            res.status(200).json({ success: true, data: result });
        } else {
            res.status(200).json({ success: false, message: "not found" });
...
router.put("/:id", auth, async (req, res, next) => {
    ...
    let find = await batteryBrand.findOne({ title: req.body.title, isDeleted: false, brandId: { $ne: req.body.brandId } });
    if (find) return res.json({ success: false, message: 'Name already exists' });
